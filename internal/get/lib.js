...
export async function generateBuildArgs(token, timezone, argsYaml, githubContext) {
  // Parse and validate inputs
  const args = parseArgs(argsYaml)
  validateInputs(token, timezone, args)
  
  // Validate configurations
  const { validConfigurations, validationErrors } = validateAndParseConfigurations(args)
