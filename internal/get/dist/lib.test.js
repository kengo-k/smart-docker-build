...
import { checkImageTagExists, createTemplateVariables, extractDockerfileConfig, extractImageNameFromDockerfile, generateImageTag, generateTagsFromTemplates, loadProjectConfig, parseGitRef, shouldBuildForChanges, validateTagsBeforeBuild, validateTemplateVariables, } from './lib.js';
// Test fixture setup
...
describe('shouldBuildForChanges', () => {
    ...
    test('should return true when changed file matches watch pattern', () => {
        ...
        expect(shouldBuildForChanges('Dockerfile', watchFiles, changedFiles)).toBe(true);
    });
    test('should return false when no changed files match watch patterns', () => {
        ...
        expect(shouldBuildForChanges('Dockerfile', watchFiles, changedFiles)).toBe(false);
    });
    test('should handle glob patterns correctly', () => {
        const watchFiles = ['src/**/*.js', '*.json'];
        // Should match
        expect(shouldBuildForChanges('Dockerfile', watchFiles, [
...
WORKDIR /app`);
    ...
    test('should extract tag configuration', () => {
        const dockerfilePath = join(testDir, 'Dockerfile');
        writeFileSync(dockerfilePath, `# image: dev-tools
...
FROM node:18`);
    ...
    test('should extract watch_files configuration', () => {
        const dockerfilePath = join(testDir, 'Dockerfile');
        writeFileSync(dockerfilePath, `# image: my-devcontainer
# imagetag_on_tag_pushed: false
...
describe('createTemplateVariables', () => {
    test('should create template variables', () => {
        const variables = createTemplateVariables('main', 'v1.0.0', 'UTC', 'abc1234567');
...
describe('checkImageTagExists', () => {
    test('should return false when API call fails', async () => {
        ...
        const exists = await checkImageTagExists(mockOctokit, 'owner', 'my-app', 'v1.0');
...
describe('validateTagsBeforeBuild', () => {
    test('should not throw when no tags exist', async () => {
        ...
        await expect(validateTagsBeforeBuild(['{tag}', 'latest'], templateVariables, mockOctokit, 'owner', 'my-app')).resolves.not.toThrow();
    });
    test('should throw when tag already exists', async () => {
        ...
        const templateVariables = { tag: 'v1.0', sha: 'abc1234' };
        await expect(validateTagsBeforeBuild(['{tag}'], templateVariables, mockOctokit, 'owner', 'my-app')).rejects.toThrow("❌ Image tag 'my-app:v1.0' already exists in registry");
    });
